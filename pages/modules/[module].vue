    ...
    <el-card>
      ...
      >
        ...
        <el-table-column
          ...
          sortable
          :key="'table' + field.label + index"
          :prop="field.label"
          min-width="300px" :label="field.label"
        >
          <template v-slot="scope">
            <el-tooltip
              placement="left-start"
              :content="`${scope.row[field.label]}`"
            >
              <span style="white-space: nowrap; overflow: hidden;">
            ...
            <div style="display: flex; flex-direction: row;">
              <el-tooltip content="View">
                <el-button
                  ...
                  size="small"
                  text
                  @click="openEditDialog(slot.row._id)"
                />
              </el-tooltip>
              <el-tooltip content="Delete">
                <el-button
                  ...
                  size="small"
                  text
                  @click="deleteSection(slot.row._id)"
                />
              </el-tooltip>
            </div>
          </template>
      ...
      <el-dialog v-model="dialogVisible" @closed="closeModal">
        ...
        <el-form>
          <div class="form-items">
            <el-form-item
              v-for="(field, index) in modulesStore.currentModule.fields"
              :key="'form' + field.label + index" :label="field.label"
            >
              <el-input
                v-if="field.type !== 'Boolean'"
                :type="field.type === 'Number' ? 'number' : 'string'"
                :model-value="sectionStore.currentSection[field.label]"
                @input="updateCurrentValue(field.label, field.type ===
                  'Number' ? +$event : $event)"
              />
              <el-switch
                v-else
                :model-value="sectionStore.currentSection[field.label]"
                @change="updateCurrentValue(field.label, $event)"
              />
            </el-form-item>
          </div>
        </el-form>
...
function openAddDialog() {
  ...
  modulesStore.currentModule.fields.map((field) => {
    sectionTemplate[field.label] = field.type === 'String' ?
