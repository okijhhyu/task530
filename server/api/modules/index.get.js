  ...
  async (ctx) => {
    try {
      ...
      if (query.sectionName) {
        // If "sectionName" is present, find a document in ModulesModel with matching sectionName
        data = await ModulesModel.findOne({ sectionName: query.sectionName })
        return data
      } else {
        // If "sectionName" is not present, retrieve all documents from ModulesModel
