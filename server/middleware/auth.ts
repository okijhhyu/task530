...
export default defineEventHandler((event) => {
  ...
  if (event.node.req.url?.includes('/api/')) {
    ...
    if (!token) {
      // If there is no valid token, set the response status code to 401 (Unauthorized)
      event.node.res.statusCode = 401

      // Write a 401 response and message to indicate the user must be signed in to access the resource
      event.node.res.writeHead(401).end('You must be signed in to access this resource.')
    }
  }
})
