...
export default defineEventHandler(async (event) => {
  // Retrieve the
  // AUTH0_COOKIE_NAME and AUTH0_CLIENT_SECRET from runtime configuration
  ...
  // Check if there's a session object in the event context;
  ...
  if (cookie) {
    ...
    try {
      const {user} = await Iron
          .unseal(cookie, AUTH0_CLIENT_SECRET, Iron.defaults);

      // If successful,
      // store the user information in the event's session context
